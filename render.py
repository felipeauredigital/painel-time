...
function renderChurn(){
  ...
  // Fee (base) = ClickUp (empresas). A VARIÁVEL vem da PLANILHA (Fee+Variável − Fee das duplas), conforme o mês filtrado.
  // Fee + Variável = Fee(ClickUp) + Variável(planilha). Fallback: variável do backend se faltar squad/mês na planilha.
  ...
  const squads=(C.squads||[]).filter(s=>s.squad!=="—"&&!hiddenSq.has(s.squad)&&teamOk(s.squad)).map(s=>{
    ...
    const feeAtv=s.feeAtivo;                                  // Fee ativo = ClickUp, como antes
    const pf=feesM[s.squad];
    const vv=pf?+Math.max(0,(pf.feeVar||0)-(pf.fee||0)).toFixed(2):(s.variavel||0);   // variável do mês (planilha)
    const feeVar=+(feeAtv+vv).toFixed(2);                     // Fee + Variável = Fee(ClickUp) + variável(planilha)
